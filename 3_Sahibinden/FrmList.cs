		...
		public void GridFill(string param = null)
		{
			...
			else
			{
				var query = from c in db.Cars
							join b in db.Brands on c.BrandId equals b.Id
							join r in db.Colors on c.ColorId equals r.Id
							where ((c.Model.Contains(txtSearch.Text) || c.Brand.Name.Contains(txtSearch.Text)) && (c.Price >= min_price && c.Price <= max_price) && (c.Year >= min_year && c.Year <= max_year) && (c.Km >= min_km && c.Km <= max_km) && (check_colors.Any(x => x == r.Name)))
							select new
							{
								Id = c.Id,
								Marka = b.Name,
